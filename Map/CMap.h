...
namespace wrj_cv {
	...
	{
	public:
		CMap(const std::string &mapFilename, const std::string &coordinateFilename, const std::string &markedMapFilename);	// 从全能地图下载器中加载地图：原始地图文件，地图的四个角坐标文件，标记遮挡的地图文件
		CMap(const cv::Mat &img, const cv::Point2d lb, const cv::Point2d rt);	// scale，未避免地图过大，先进行缩放

		void markGPSPt(const cv::Point2d &GPSPt);		// 在地图中标记GPS点
